...
router.get(
  ...
  [
    ...
    query('search').optional().trim().isLength({ max: 100 }).withMessage('Search query must be less than 100 characters'),
    query('departmentId').optional().isString().withMessage('Department ID must be a string'),
    query('isActive').optional().isBoolean().withMessage('isActive must be a boolean'),
    query('employmentType').optional().isIn(['FULL_TIME', 'PART_TIME', 'CONTRACT', 'INTERN', 'TEMPORARY']).withMessage('Invalid employment type')
  ],
  auditRead('Employee'),
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      if (!req.organization) {
        return res.status(400).json({ success: false, error: 'Organization context not found' });
      }
      ...
      const [employees, totalCount] = await Promise.all([
        db.employee.findMany({
          ...
          include: {
            ...
            _count: { select: { attendanceRecords: true, leaveRequests: true } }
          },
...
router.post(
  ...
  [
    ...
    body('employeeId').trim().isLength({ min: 1, max: 20 }).withMessage('Employee ID must be between 1 and 20 characters'),
    ...
    body('employmentType').optional().isIn(['FULL_TIME', 'PART_TIME', 'CONTRACT', 'INTERN', 'TEMPORARY']).withMessage('Invalid employment type'),
    ...
    body('pin').optional().isLength({ min: 4, max: 8 }).withMessage('PIN must be between 4 and 8 characters')
  ],
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const {
        firstName, lastName, employeeId, email, phone, dateOfBirth, position,
        departmentId, employmentType, hireDate, salary, hourlyRate, currency, pin
      } = req.body;
      const orgId = req.organization.id;

      // Employee ID check
      const existingEmployee = await db.employee.findUnique({
        where: { organizationId_employeeId: { organizationId: orgId, employeeId } }
      });
      if (existingEmployee) {
        return res.status(400).json({ success: false, error: 'Employee ID already exists in this organization' });
      }
      // Email check
      if (email) {
        const existingEmail = await db.employee.findFirst({ where: { organizationId: orgId, email } });
        if (existingEmail) {
          return res.status(400).json({ success: false, error: 'Email address already in use by another employee' });
        }
      }
      // Department check
      if (departmentId) {
        const department = await db.department.findFirst({ where: { id: departmentId, organizationId: orgId } });
        if (!department) {
          return res.status(400).json({ success: false, error: 'Department not found or not accessible' });
        }
      }
      const employeeData = {
      ...
      const employee = await db.employee.create({
        ...
        include: {
          department: { select: { id: true, name: true } },
          organization: { select: { id: true, businessName: true, orgCode: true } }
        }
      });

      logger.info(`Employee created: ${employeeId} by ${req.user?.email} in organization ${req.organization.orgCode}`, {
        organizationId: req.organization.id, employeeId, firstName, lastName
      });
...
router.get(
  '/:id',
  requireManager,
  [param('id').isString().isLength({ min: 1 }).withMessage('Valid employee ID is required')],
  auditRead('Employee'),
  async (req: Request, res: Response): Promise<Response> => {
    try {
      const errors = validationResult(req);
      ...
      const employee = await db.employee.findFirst({
        ...
        include: {
          ...
          attendanceRecords: {
            ...
            take: 10,
            select: {
              id: true, date: true, checkInTime: true, checkOutTime: true, hoursWorked: true, status: true,
              isLate: true, isEarlyLeave: true, overtimeHours: true
            }
          },
          leaveRequests: {
          ...
          _count: { select: { attendanceRecords: true, leaveRequests: true, payrollRecords: true } }
        }
      });
      if (!employee) {
      ...
      return res.status(500).json({ success: false, error: 'Failed to retrieve employee details' });
    }
  }
);
...
router.put(
  ...
  [
    ...
    body('dateOfBirth').optional().isISO8601().withMessage('Date of birth must be a valid date'),
    body('terminationDate').optional().isISO8601().withMessage('Termination date must be a valid date'),
    body('pin').optional().isLength({ min: 4, max: 8 }).withMessage('PIN must be between 4 and 8 characters')
  ],
  auditUpdate('Employee'),
  async (req: Request, res: Response): Promise<Response> => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      if (updateData.email && updateData.email !== existingEmployee.email) {
        const emailConflict = await db.employee.findFirst({
          where: { organizationId: req.organization.id, email: updateData.email, id: { not: id } }
        });
        if (emailConflict) {
          return res.status(400).json({ success: false, error: 'Email address already in use by another employee' });
        }
      ...
      if (updateData.departmentId) {
        const department = await db.department.findFirst({
          where: { id: updateData.departmentId, organizationId: req.organization.id }
        });
        if (!department) {
          return res.status(400).json({ success: false, error: 'Department not found or not accessible' });
        }
      }
      // Clean/process update data
      ...
      const updatedEmployee = await db.employee.update({
        where: { id },
        data: { ...processedUpdateData, updatedAt: new Date() },
        include: {
          department: { select: { id: true, name: true } },
...
router.delete(
  ...
  [
    param('id').isString().isLength({ min: 1 }).withMessage('Valid employee ID is required'),
    query('permanent').optional().isBoolean().withMessage('Permanent flag must be a boolean')
  ],
  auditDelete('Employee'),
  async (req: Request, res: Response): Promise<Response> => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      if (permanent === 'true') {
        // Only allow permanent delete if no attendance records
        const attendanceCount = await db.attendanceRecord.count({ where: { employeeId: id } });
        ...
        logger.info(`Employee permanently deleted: ${employee.employeeId} by ${req.user?.email} in organization ${req.organization.orgCode}`, {
          organizationId: req.organization.id, employeeId: employee.employeeId
        });
        ...
        const updatedEmployee = await db.employee.update({
          where: { id },
          data: { isActive: false, terminationDate: new Date(), updatedAt: new Date() }
        });
...
router.post(
  ...
  [
    ...
    body('employees.*.employeeId').trim().isLength({ min: 1, max: 20 }).withMessage('Each employee must have a valid employee ID'),
    body('employees.*.firstName').trim().isLength({ min: 1, max: 50 }).withMessage('Each employee must have a valid first name'),
    body('employees.*.lastName').trim().isLength({ min: 1, max: 50 }).withMessage('Each employee must have a valid last name')
  ],
  auditCreate('Employee'),
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      for (const employeeData of employees) {
        try {
          ...
          const existing = await db.employee.findFirst({
            where: { organizationId: req.organization.id, employeeId: employeeData.employeeId }
          });
          if (existing) {
            results.failed.push({
          ...
          if (employeeData.departmentId) {
            const department = await db.department.findFirst({
              where: { id: employeeData.departmentId, organizationId: req.organization.id }
            });
            if (!department) {
              results.failed.push({
                employeeId: employeeData.employeeId,
      ...
      return res.status(500).json({ success: false, error: 'Failed to process bulk import' });
    }
  }
);
